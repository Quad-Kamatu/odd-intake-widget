...
class OddIntakeElement extends HTMLElement {
  ...
  _onMessage(e) {
    ...
    // On submit success: snap height and keep nudging for ~2s to ensure the host collapses
    if (d.type === 'ODD_FORM_SUCCESS') {
      if (typeof d.height === 'number') this._setHeight(d.height);
      if (this._successNudge) clearInterval(this._successNudge);
      const t0 = Date.now();
      this._successNudge = setInterval(() => {
        // Re-apply the last known height to defeat any host-side throttle/debounce
...
// Define both tags for compatibility
